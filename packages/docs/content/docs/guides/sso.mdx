...
AIOStreams can delegate operator login to any OpenID Connect provider — Authelia, Authentik, Keycloak, Entra ID, Google, and so on.
...
**Point AIOStreams at the provider.** Only four settings are required. The issuer is the root URL; `/.well-known/openid-configuration` is discovered automatically.
...
AIOSTREAMS_OIDC_CLIENT_SECRET=<secret>
```
...
<Callout type="info">
  Today `admin` is the only permission that changes what a signed-in session can
  do, because it is what the dashboard checks. Mapping a group to `proxy` or
  `sabnzbd` is recorded on the session but has no effect yet, since those
...
<Tabs items={['Authelia', 'Authentik', 'Keycloak', 'Google']}>
<Tab value="Authelia">
...
| `oidc_discovery_failed`  | Provider unreachable                          | Check the issuer URL and that AIOStreams can resolve it                                                                                            |
