...
**Point AIOStreams at the provider.** Only four settings are required. The issuer is the provider's root URL; `/.well-known/openid-configuration` is discovered automatically.
