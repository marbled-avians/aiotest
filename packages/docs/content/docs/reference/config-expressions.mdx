...
A **variant** is a named script that adjusts your configuration at request time. Variants live inside one config and are selected with a query parameter on the manifest URL, so a single UUID can serve several differently-behaving addons.

The scripts are written in the **Config Expression Language (CEL)**. Where the [Stream Expression Language](/docs/reference/stream-expressions) is evaluated against a list of streams, CEL is declarative: a list of edits applied to your configuration, top to bottom.
...
| ------------------- | --------------------------------- | --------------------------------- |
| Configs to maintain | Two, each with its own UUID       | One                               |
| Control             | Per-section merge strategies      | A script                          |
| Best for            | Sharing a base config with others | Running your own config many ways |
...
Clients should keep the query string when they request streams, catalogs, meta and subtitles, so the variant applies for the whole session.
...
```
.../manifest.json?v=phone,rd2
```

An unknown or disabled id is an error, surfaced as an error stream, rather than a silent fall back to the base config.

The selector also works on ChillLink, the Seanime extension URLs, and the Search and Newznab/Torznab APIs (as `&v=`).

<Callout type="warn">
The configure page always edits the **base** config, even if opened with `?v=`.
</Callout>
