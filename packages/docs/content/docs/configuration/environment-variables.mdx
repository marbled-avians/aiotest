...
| `AIOSTREAMS_OIDC_ISSUER`                  | Issuer                  | string  | —                              | OIDC issuer URL. Discovery is performed against &#123;issuer&#125;/.well-known/openid-configuration. Register &lt;BASE_URL&gt;/api/v1/auth/oidc/callback as an allowed redirect URI with your provider.                                                                                                                                                                                                            |
...
| `AIOSTREAMS_OIDC_GROUP_PERMISSIONS`       | Group Permissions       | map     | —                              | Maps groups to permissions. Comma-separated `group=perm\|perm` entries (valid: admin, proxy, service, sabnzbd, none). Group names containing "," or "=" (such as LDAP DNs) must use the JSON object form instead, e.g. &#123;"cn=admins,ou=groups,dc=example,dc=com":"admin"&#125;.                                                                                                                                |
